                ...
                allpost.map((post) => {
                    return (
                        <div key={post._id} className="card home-card">
                            <h6 style={{ padding: "12px 10px 0px 12px"}} className='post-header'>
                                <img className="profile-image" src={post.postedBy.profilepic} style={{ width: '35px', height: '35px' }} />
                                <span className='post-user'><Link to={state._id != post.postedBy._id ? `/profile/${post.postedBy._id}` : "/profile"}><b>{post.postedBy.name}</b></Link></span>
                                {
